...
do i=1,M
	...
	do j=1,N
		spge(i,j)=(1.0/3600.0)*(sin((0.5*4.0*atan(1.0)/(N_vert*hz-spge_h*N_vert*hz))*(zt(i)&
										-N_vert*hz*spge_h))**2)
	enddo
	endif
enddo
...
subroutine sponge_layer(u,t,M,N)
...
!	open(unit=12,status='replace',file='test59.txt',form='formatted')
!	do i=1,M-1
!		write(12,*) (f(i,j), j=1,N)
!	enddo
! 	close(12)

!	open(unit=12,status='replace',file='test60.txt',form='formatted')
!	do i=1,M-1
!		write(12,*) sign((1.0/N)*sum(f(i,:)),(1.0/N)*sum(f(i,:)))
!	enddo
! 	close(12)
...
subroutine calc_ql(ql,qt,qvs,M,N)
...
subroutine ls_force_therm(ls_fBu,ls_fBs,Bu,Bs,M,N)
...
!do i=1,M
!	ls_fBu(i,:)=sum(Bu(i,:))/(N*therm_relax)!sign(sum(Bu(i,:))/(N*therm_relax),sum(Bu(i,:))/(N*therm_relax))
!	ls_fBs(i,:)=sum(Bs(i,:))/(N*therm_relax)!sign(sum(Bs(i,:))/(N*therm_relax),sum(Bs(i,:))/(N*therm_relax))
!enddo

do i=1,M
	do j=1,N
		ls_fbs(i,j)=max_moist*(1.0/therm_relax)*(1.0/(max_moist_z*N_vert*hz))*zt(i)*&
...
do i=1,M
	do j=1,N
		ls_fbu(i,j)=max_t*(1.0/therm_relax)*(1.0/(max_t_z*N_vert*hz))*zt(i)*&
